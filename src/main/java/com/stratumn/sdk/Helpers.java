...
public class Helpers {
   ...
   public static <T> Map<String, Property<FileWrapper>> extractFileWrappers(T data) {
      return extractObjects(data, (X) -> FileWrapper.isFileWrapper(X), (T) -> {
         return FileWrapper.fromObject((Object) T);
      });
   }
   ...
   private static <T, V extends Identifiable> void extractObjectsImpl(T data, Object parent, String path,
      ...
      if (predicate.test(data)) {
         ...
         if (data instanceof JsonObject) { // loop over subelements
            for (Entry<String, JsonElement> element : ((JsonObject) data).entrySet()) {
               extractObjectsImpl((T) element.getValue(), data,
                     path.isEmpty() ? element.getKey() : path + "." + element.getKey(), idToObjectMap, predicate,
                     reviver);
            }
         } else if (data instanceof JsonArray) { // loop over indexes .
            int idx = 0;
         ...
         for (Field field : fields) {
            ...
            extractObjectsImpl(value, data, path.isEmpty() ? field.getName() : path + "." + field.getName(),
      ...
      for (Property<V> propertyElement : propertyList) {
         ...
         if (parent instanceof JsonElement) {
            if (parent instanceof JsonObject)
               ...
               ((JsonArray) parent).set(index,
                     JsonHelper.objectToObject(propertyElement.getValue(), JsonElement.class));
            }
            ...
            try {
               ...
               // in java there is currently no way of changing the type of a field. A field
               // has to be of type identifiable
