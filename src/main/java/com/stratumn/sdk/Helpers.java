...
public class Helpers
{
   ...
   * authenticate to retrieve a long lived auth token.
   ...
   {
      return extractObjects(data, (T) -> FileRecord.isFileRecord(T), 
         (T) -> {return FileRecord.fromObject((Object)T); });
   }
   ...
   {
      return extractObjects(data, (X) -> FileWrapper.isFileWrapper(X),
         (T) -> {return FileWrapper.fromObject((Object)T); }
         );
   }
    ...
    */
      private static <T, V extends Identifiable> Map<String, Property<V>> extractObjects  (T data, 
      ...
      private static <T , V extends Identifiable> void extractObjectsImpl (T data, Object parent, String path,  
         Map<String,  Property<V>> idToObjectMap,
         Predicate<T> predicate ,
         Function<T, V> reviver )
      {
         if (data==null) return;
         ...
         else if ( data instanceof JsonElement)
         {
            ...
            {  //loop over subelements
               for(Entry<String, JsonElement> element : ((JsonObject) data).entrySet())
               {
            ...
            {  //loop over indexes .
               int idx=0;
               for(JsonElement value : ((JsonArray) data))
               {
                  extractObjectsImpl((T)value, data, path + "[" + idx + "]", idToObjectMap, predicate, reviver);
                  idx++;
               }  
            }
         }
         ...
         else if ( data instanceof Map )
         {
            ...
            if(data instanceof Object  && !data.getClass().getName().startsWith("java."))
          {
             ...
             Class<?> clazz = data.getClass();
                ...
                {
                  if(field.getType().isPrimitive() || field.isEnumConstant() || field.isSynthetic()
                  ...
                  extractObjectsImpl(value, data, path.isEmpty() ? field.getName() : path + "." + field.getName(), idToObjectMap, predicate,
                     reviver);
             }
       ...
       * Identifiable interface and fields/arrays are defined using that interface.
       * @param data
       * @param propertyList
       * @return
           ...
           //read the key name from the path
            ...
            {   
               if (parent instanceof JsonObject)
                  ((JsonObject) parent).add(key, JsonHelper.objectToObject(propertyElement.getValue(),JsonObject.class));
               else if (parent instanceof JsonArray)
               {
                  int index = extractIndexFromKey(key); 
                  ((JsonArray) parent).set(index, JsonHelper.objectToObject(propertyElement.getValue(),JsonElement.class));
               }   
            }
             ...
             {  
                ...
                {   
                  ...
                  //object could be an identifiable or it could be a deserialized map
                   if (parent.getClass().getComponentType().isAssignableFrom(Map.class))
                  ...
                  try
                  {
                     ...
                     else
                     {
                        if(!field.getType().isAssignableFrom(propertyElement.getValue().getClass()))
                              ...
                              "Field " + key + " of type " + field.getType() + " is not assignable from " + propertyElement.getValue().getClass());
      ...
      private static Pattern KeyIndex = Pattern.compile("\\[(\\d+)\\]|\\((\\d+)\\)$");
      /***
       * Extracts the index on the key of the form [i] or (i)
       * @param key
       * @return
       * @throws TraceSdkException 
       */
      private static int extractIndexFromKey(String key) throws TraceSdkException
      {
         //get the index of the value in the array or collection
         //read the index from the path  
         Matcher lastIndexMatcher = KeyIndex.matcher(key);
         int index;
